...
class ProposalResult[
    ...
    """Output of a full-population SMC proposal kernel.

    ``ancestor_log_prob`` declares a normalized measure over input particles.
    The weighted output ancestors must represent that measure: in expectation,
    the total ``exp(resampled_log_w)`` assigned to an input ancestor equals its
    ``exp(ancestor_log_prob)``. This includes deterministic identity mappings.
    """

    particles: ParticleT
    resampled_context: FilterContext[
        ParticleT,
        ObservationT,
        ConditionT,
