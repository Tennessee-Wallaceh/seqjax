...
def test_positional_embedder_builds_and_embeds_sequence_context() -> None:
    ...
    observations, conditions, parameters = _make_dummy_inputs(target_posterior, sample_length)
...
def test_positional_embedder_sequence_mode_requires_sequence_start() -> None:
    ...
    embedding = vi.registry._build_embedder(
        vi.registry.PositionalEmbedderConfig(position_mode="sequence"),
        target_posterior=target_posterior,
        sequence_length=11,
        sample_length=sample_length,
...
def test_positional_embedder_sequence_mode_uses_global_sequence_position() -> None:
    ...
    embedding = vi.registry._build_embedder(
        vi.registry.PositionalEmbedderConfig(
